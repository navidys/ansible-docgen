PKG_PATH = "github.com/navidys/ansible-docgen"
GO := go
...
.PHONY: binary
binary: $(TARGET)  ## Build ansible-docgen binary
	@true
...
$(TARGET): $(SRC)
	...
	@env CGO_ENABLED=0 $(GO) build $(BUILDFLAGS) -o $(BIN)/$(TARGET)

.PHONY: binary-win
binary-win:  ## Build ansible-docgen.exe windows binary
	@mkdir -p $(BIN)/windows/
	@echo "running go build for windows"
	@env CGO_ENABLED=0 GOOS=windows GOARCH=$(GOARCH) $(GO) build $(BUILDFLAGS) -o $(BIN)/windows/$(TARGET).exe

.PHONY: binary-darwin
binary-darwin: ## Build ansible-docgen for darwin
	@mkdir -p $(BIN)/darwin/
	@echo "running go build for darwin"
	@env CGO_ENABLED=0 GOOS=darwin GOARCH=$(GOARCH) $(GO) build $(BUILDFLAGS) -o $(BIN)/darwin/$(TARGET) -tags
...
.PHONY: install
install:    ## Install ansible-docgen binary
	@install ${SELINUXOPT} -D -m0755 $(BIN)/$(TARGET) $(DESTDIR)/$(TARGET)

.PHONY: uninstall
uninstall:  ## Uninstall ansible-docgen binary
	@rm -f $(DESTDIR)/$(TARGET)
...
.PHONY: install.tools
install.tools: .install.ginkgo .install.pre-commit .install.codespell .install.golangci-lint ## Install needed tools
...
package-install: package  ## Install rpm package
	...
	/usr/bin/ansible-docgen version
